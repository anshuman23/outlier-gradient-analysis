...
def compute_influence(args, data, model):
    ...
    if args.infl_on_train != 'y':
        util_loss_total_grad, acc_loss_indiv_grad = model.grad(data.x_val, data.y_val)
    else:
        util_loss_total_grad, acc_loss_indiv_grad = model.grad(data.x_train, data.y_train)
    ...
    return util_pred_infl, train_indiv_grad
...
if __name__ == "__main__":
    ...
    loo_influences, grads = compute_influence(args, data, model)

    test_evaluator = Evaluator(data.s_test, "test")
    test_res = test_evaluator(data.y_test, model.pred(data.x_test)[1])


    od_idxs = np.array([149, 145, 78, 25, 119, 41, 7, 15, 16, 62])

    outlier_scores = do_outlier_analysis(grads) #Both first and second order gradient information is used
    ...
    indices = [149, 145, 78, 25, 119, 41, 7, 15, 16, 62] #np.load('data/half_moons/od_idxs.npy').tolist()
    print(indices)
    ...
    plot_outlier_analysis(axs[3], loo_influences, outlier_scores, od_idxs) #Plot using loo accuracy values
    ...
    plt.tight_layout()
    plt.savefig('without_hess_figs/toy_full_LR.png', dpi=300, bbox_inches='tight')



    #delete_pred_outliers_exp(args, data, outlier_scores)
    # od_idxs = [149, 145, 78, 25, 119, 41, 7, 15, 16, 62]
