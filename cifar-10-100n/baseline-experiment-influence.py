...
org_acc, new_acc_iden, new_acc_datainf, new_acc_lissa = [], [], [], []
...
if args.dataset == 'cifar10':
    ...
    grads = np.zeros((len(temp_dl), 51200)) #hardcoding gradient dimension for now for ease
...
# obtain dict of gradients
grads_dict = {i: {'main': None} for i in range(len(grads))}
for i in range(len(grads)):
    if args.dataset == 'cifar10':
        grads_dict[i]['main'] = torch.from_numpy(grads[i].reshape(10,512))
    else:
        grads_dict[i]['main'] = torch.from_numpy(grads[i].reshape(100, 512))
...
datainf_idxs = find_idxs(inf_eng.IF_dict['proposed'])
lissa_idxs = find_idxs(inf_eng.IF_dict['LiSSA'])

# remove samples from original dataset
train_dl_iden = sample_remove_dataloader(train_dataset, iden_idxs)
train_dl_datainf = sample_remove_dataloader(train_dataset, datainf_idxs)
train_dl_lissa = sample_remove_dataloader(train_dataset, lissa_idxs)

# retrain models on new dataset
...
for epoch in range(args.n_epoch):
    ...
    adjust_learning_rate(optimizer, epoch, alpha_plan)
    model.train()
    train_acc = train(epoch, train_dl_iden, model, optimizer)
    test_acc = evaluate(test_loader=test_loader, model=model)
    print('train acc on train images is ', train_acc)
    print('test acc on test images is ', test_acc)
    new_acc_iden.append(test_acc)
...
for epoch in range(args.n_epoch):
    ...
    adjust_learning_rate(optimizer, epoch, alpha_plan)
    model.train()
    train_acc = train(epoch, train_dl_datainf, model, optimizer)
    test_acc = evaluate(test_loader=test_loader, model=model)
    print('train acc on train images is ', train_acc)
    print('test acc on test images is ', test_acc)
    new_acc_datainf.append(test_acc)
...
for epoch in range(args.n_epoch):
    ...
    adjust_learning_rate(optimizer, epoch, alpha_plan)
    model.train()
    train_acc = train(epoch, train_dl_lissa, model, optimizer)
    test_acc = evaluate(test_loader=test_loader, model=model)
    print('train acc on train images is ', train_acc)
    print('test acc on test images is ', test_acc)
    new_acc_lissa.append(test_acc)



# new models on trimmed data has been trained
org_acc = np.max(org_acc)
new_acc_iden = np.max(new_acc_iden)
new_acc_datainf = np.max(new_acc_datainf)
new_acc_lissa = np.max(new_acc_lissa)

ending = time.time()
print("\nEntire experiment concluded in {} seconds.\n".format(ending-beginning))
print("\nOriginal Accuracy: {} || New Accuracy Identity: {} || New Accuracy DataInf: {} || New Accuracy LiSSA: {}\n".format(org_acc, new_acc_iden, new_acc_datainf, new_acc_lissa))
