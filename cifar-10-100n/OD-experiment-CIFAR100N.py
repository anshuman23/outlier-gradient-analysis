...
grads_embed = SparseRandomProjection(n_components='auto').fit_transform(grads) #grads_embed = TSNE(n_components=2).fit_transform(grads)
end_time = time.time()
...
start_time = time.time()
#clf = IForest(contamination=0.05) #contaimination=0.05 works best so far
