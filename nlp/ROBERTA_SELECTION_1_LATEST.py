...
##############################################
# OBTAIN SELF INFLUENCE BASELINES

grads = grads_dict_to_arr(tr_grad_dict)
grads_dict = {i: {'main': None} for i in range(len(grads))}
for i in range(len(grads)):
    grads_dict[i]['main'] = torch.from_numpy(grads[i].reshape(2,1024))


self_influences_lissa = []
self_influences_datainf = []
...
for i in range(len(grads)):
    ...
    influence_engine.preprocess_gradients(temp_gr_d, temp_gr_d, noise_index) #Note no validation set gradients will be used
    influence_engine.compute_hvps(compute_accurate=False)
    influence_engine.compute_IF()

    self_influences_datainf.append(influence_engine.IF_dict['proposed'][0])
    self_influences_lissa.append(influence_engine.IF_dict['LiSSA'][0])

self_influences_datainf = np.array(self_influences_datainf)
self_influences_lissa = np.array(self_influences_lissa)

#print(self_influences_lissa)
print(self_influences_datainf.shape, self_influences_lissa.shape)

self_datainf_idxs = find_idxs(self_influences_datainf)
self_lissa_idxs = find_idxs(self_influences_lissa)


##############################################


# OBTAIN ALL OTHER INFLUENCE BASELINES
influence_engine = IFEngine()
...
# OBTAIN OUTLIER GRADIENT ANALYSIS
grads = grads_dict_to_arr(tr_grad_dict)
print(grads.shape)
...
#od_idxs = np.argsort(norm2)[::-1]
#od_idxs = list(od_idxs[:int(0.3*len(norm2))])

# USE L1 NORM?
#norm1 = np.sum(np.abs(grads), axis=1)
#od_idxs = np.argsort(norm1)[::-1]
#od_idxs = list(od_idxs[:int(0.3*len(norm1))])

print(len(od_idxs), len(iden_idxs), len(datainf_idxs), len(lissa_idxs))


#RETRAIN THE MODEL FOR EACH BASELINE AND OUTLIER TRIMMING
accuracies = {'outlier': [], 'identity': [], 'lissa': [], 'datainf': [], 'self_lissa': [], 'self_datainf': []}
...
self_datainf_trainloader = sample_remove_dataloader(tokenized_datasets['train'], batch_size, self_datainf_idxs)
self_lissa_trainloader = sample_remove_dataloader(tokenized_datasets['train'], batch_size, self_lissa_idxs)
...
for _ in range(3):
    ...
    accuracies['self_datainf'].append(retrain_model(self_datainf_trainloader, task))
    accuracies['lissa'].append(retrain_model(lissa_trainloader, task))
    accuracies['self_lissa'].append(retrain_model(self_lissa_trainloader, task))
